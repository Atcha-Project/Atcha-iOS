...
- **인증은 게스트 전용(MVP, 2026-09)** — 소셜 로그인(AuthFeature·카카오 SDK·Apple 로그인)은 제거됐다(복구가 필요하면 PR #361 히스토리). 토큰이 없으면 스플래시에서 `IssueGuestSessionUseCase`가 키체인 보관 기기 UUID(`KeychainDeviceIdentifier`, 재설치해도 유지)로 **`POST /auth/guest`(클라 제안 계약, 서버 미구현·합의 대기)** 를 호출해 토큰을 채택하고, 실패하면 스플래시 재시도 UI를 띄운다. refresh 확정 만료 시에는 화면을 그대로 두고 같은 기기 ID로 조용히 재발급한다 — "같은 deviceId = 같은 게스트 회원"이 서버 계약 전제라 알람·집 주소는 정리하지 않는다. 로그아웃·회원탈퇴 개념은 없다. `SettingsFeature`(홈 상단 톱니바퀴: 우리집 설정·약관·피드백·버전)가 `GET /members/me`·홈주소 PATCH를 쓰고, 홈주소 변경은 `GET /locations/is-service-region` 통과 후 PATCH, 알림 빈도는 미노출. 앱 버전은 `GET /app/version`(무토큰, 1.5초 예산·실패 시 통과)으로 권장 업데이트 팝업만. `DevDemoFallbacks`와 "DEV 건너뛰기"는 제거됨 — DEV에 남은 것은 막차 변경 주입용 `DevChangeSimulator`뿐이라 서버 실패가 DEV에서도 그대로 표면화된다.

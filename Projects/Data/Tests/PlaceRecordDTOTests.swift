...
struct PlaceRecordDTOTests {
    ...
    func roundTrip_placeToRecordToEntity_preservesFields() throws {
        ...
        let encoded = try JSONEncoder().encode(PlaceRecordDTO(place))
        let decoded = try JSONDecoder().decode(PlaceRecordDTO.self, from: encoded)

...
final class SearchViewModel {
    ...
    /// 화면 상태 전부. 입력 슬롯을 밖에 두고 별도 콜백으로 흘리면 상태 채널이 둘이
    /// 되어, "지금 화면이 무엇을 보여 주는가"를 한 타입으로 말할 수 없다.
    ///
    /// 슬롯을 따로 흘리던 이유(매 키 입력마다 전체를 push하면 `UITextField` 커서가
    /// 튄다)는 정당하지만, **diff는 계약이 아니라 VC의 일이다** — VC가 직전 상태를
    /// 들고 바뀐 것만 반영한다.
    struct State: Equatable {
        var fields: FieldsViewData
    ...
    private(set) var state = State(
        fields: FieldsViewData(departureText: "", arrivalText: "", activeField: .departure)
    ) {
    ...
    init(
        ...
        state.fields.activeField = (initialField == .arrival) ? .arrival : .departure
    }
    ...
    func fieldDidBeginEditing(_ field: Field) {
        state.fields.activeField = field
    ...
    func didSelectListItem(at index: Int) {
        ...
        confirm(listedPlaces[index], in: state.fields.activeField)
    }

    func didDeleteRecent(at index: Int) {
        guard case .recent = state.content, listedPlaces.indices.contains(index) else { return }
    ...
    func didTapMore() {
        guard case .routes = state.content, !availableRoutes.isEmpty else { return }
        isExpanded.toggle()
        state.content = .routes(
            RouteResultsViewData(entities: availableRoutes, isExpanded: isExpanded, now: now())
        )
    }

    func didSelectRoute(at index: Int) {
        guard case .routes = state.content, availableRoutes.indices.contains(index) else { return }
    ...
    private func prefillDepartureWithCurrentLocation() {
        ...
        locationTask = Task { [weak self] in
            ...
            guard self.departure == nil, self.state.fields.departureText.isEmpty else { return }
            self.departure = Place(name: "현재 위치", address: "", coordinate: coordinate)
            var newFields = self.state.fields
            newFields.departureText = "현재 위치"
            newFields.activeField = .arrival
            self.state.fields = newFields
    ...
    private func searchRoutes() {
        ...
        routeTask = Task { [weak self] in
            ...
            do {
                ...
                guard !Task.isCancelled else { return }
                self?.state.content = .failed(message: "막차를 찾지 못했어요")
            }
        }
    }

...
| # | 항목 | 클라 측 잔여 작업 |
|---|---|---|
| **S1** | ~~익명 인증 발급 엔드포인트 (#2)~~ — **소셜 로그인으로 대체 완료(2026-09)**: 익명 발급 개념을 접고 카카오·애플 로그인(`GET /auth/check`·`/auth/login`, 레거시 실측 계약)이 최초 토큰 소스가 됐다. 강제 로그인(게스트 없음), refresh 확정 거절 시 `loginRequired` 표면화 | 잔여: 카카오 콘솔에 `com.atcha.iOS.v2` 등록 + `TUIST_KAKAO_APP_KEY` 주입, Apple 콘솔 capability, 서버의 V2 번들 audience 허용 확인, `DevDemoFallbacks` 제거(실서버 검증 후) |

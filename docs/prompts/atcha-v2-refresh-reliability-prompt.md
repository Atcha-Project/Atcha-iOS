...
**실기기 잔여**: 실서버 오프라인·타임아웃 실측(스플래시 문구 분기 실표출 포함 — 현 DEV는 issuerNotConfigured 삼킴으로 재시도 화면 도달 불가, S1 이후 확인). 사일런트 푸시 실수신 경로(기존 항상-잔여 항목). (해당 시) pull-to-refresh 당김 제스처 실동작.
